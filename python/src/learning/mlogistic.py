...
class LogisticRegression:
    ...
    def negative_log_likelihood(self,input,output):
        ...
        return -T.mean(T.log(self.p_y_given_x)[T.arange(output.shape[0]),output])
    def errors(self,input,output):
        self.compute_input(input)
        if output.dtype.startswith('int'):
            return T.mean(T.neq(self.y_pred,output))
        else:
...
def load_data(dataset):
    ...
    return rval
def test_minst(learning_rate=0.13,n_epocks=1000,dataset='d:/mnist.pkl.gz',batch_size=600,n_epoch=1000):
    ...
    test_model=theano.function(inputs=[index],outputs=classifier.errors(x, y),
                               givens={
